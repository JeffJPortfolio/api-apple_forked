...
public class UserController {
	...
	public JsonResult userMypageForm(HttpServletRequest request) {
		...
		if (userNum != -1) { // 토큰정상
			unionVo unionVo = userService.exeMypageForm(userNum); // userNum는 유저번호
	...
	public JsonResult purchaseList(HttpServletRequest request) {// 토큰에서 꺼낸다
		...
		List<unionVo> purchaseList = userService.exeUserPurchase(userNum);
		System.out.println("userNum" + userNum);
		System.out.println("purchaseList" + purchaseList);
		return JsonResult.success(purchaseList);
